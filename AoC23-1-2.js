...
numList = ["one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "1", "2", "3", "4", "5", "6", "7", "8", "9"];

input = string.split("\n");
total = 0;

function NumsFromString(string) {
    arr = [];
    index = 0;
    biggest = -1;
    smallest = string.length;
    
    for (i=0;i<numList.length;i++) { // get the first instance of a number in string
        index = string.indexOf(numList[i]);
    ...
    for (i=0;i<numList.length;i++) { // get the last instance of number in string
        index = string.lastIndexOf(numList[i]);
        if ((index != -1) && (index > biggest)) {
            arr[1] = numList[i];
            biggest = index;
        }
    }
    
    for (i=0;i<arr.length;i++) { // convert "one" to 1 etc.
        if (isNaN(arr[i])) {
            arr[i] = numList[numList.indexOf(arr[i])+9];
        }
    }
...
for (string=0;string<input.length;string++) {
    total += NumsFromString(input[string]);
}
