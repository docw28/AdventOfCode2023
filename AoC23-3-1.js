...
input = input.split("\n");                  // Turning input into a XY-navigable array of arrays
for (line=0;line<input.length;line++) {
    input[line] = input[line].split("");
    validNums[line] = [];
}

function isNumTouchingSymbol(x, y) {
    for (yOff=-1;yOff<2;yOff++) {
        for (xOff=-1;xOff<2;xOff++) {           // Get the 3x3 grid around the character
            if (
                !yOff&&!xOff ||                 // But not the character itself
                y+yOff < 0 || y+yOff > input.length - 1 ||  // And not if it's outside the whole grid
                x+xOff < 0 || x+xOff > input[y+yOff].length - 1
            ) { 
                continue;
            } else {
                try {
...
function populateValidNums(arr) {
    for (y=0;y<arr.length;y++) {
        for (x=0;x<arr[y].length;x++) {
            if (!isNaN(input[y][x]) && (arr[y][x-1] || arr[y][x+1])) {
                validNums[y][x] = 1;
            }
        }
    }
...
for (row=0;row<input.length;row++) {
    for (column=0;column<input[row].length;column++) {
        validNums[column][row] = isNumTouchingSymbol(row, column);
    }
}

populateValidNums(validNums);
populateValidNums(validNums);

console.log(engineParts(input, validNums));