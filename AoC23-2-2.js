...
function gamePower(input) {    // Returns the game number is possible, otherwise returns 0
    input = input.split(/[:;]+/);   // Formatting input and extracting game number
    game = Number(input.shift().slice(5));
    Nred = Ngreen = Nblue = 0;
    
    for (i=0;i<input.length;i++) {  // Separating games into sets of numbers and colours
        ...
        for (j=0;j<input[i].length;j++) {
            ...
            switch(arr[1]) {
                case "red":
                    Nred = Math.max(Nred, arr[0]);
                    break;
                case "blue":
                    Nblue = Math.max(Nblue, arr[0]);
                    break;
                case "green":
                    Ngreen = Math.max(Ngreen, arr[0]);
            }
        }
    }
    return Nred * Nblue * Ngreen;
}

for (line=0;line<input.length;line++) {
    total += gamePower(input[line]);
}
