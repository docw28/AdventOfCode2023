...
function isGamePossible(input) {    // Returns the game number is possible, otherwise returns 0
    ...
    for (i=0;i<input.length;i++) {  // Separating games into sets of numbers and colours
        ...
        for (j=0;j<input[i].length;j++) {
            ...
            switch (input[i][j][1]) {
                case "red":
                    if (input[i][j][0] > Nred) { return 0 }
                case "green":
                    if (input[i][j][0] > Ngreen) { return 0 }
                case "blue":
                    if (input[i][j][0] > Nblue) { return 0 }
            }
        }
    }
    return game;
...
for (line=0;line<input.length;line++) {
    total += isGamePossible(input[line]);
}
